...
/**
 * Check if a nav item has a dropdown
 * @param {Element} li - The list item
 * @returns {boolean}
 */
function hasDropdown(li) {
...
function buildSection(section, style) {
  ...
  if (style === SECTION_CLASSES.MAIN_NAV) {
    ...
    // Check for logo (p containing picture/img as first element)
    ...
    if (firstChild?.tagName === 'P' && firstChild.querySelector('picture, img')) {
      ...
      children.shift(); // Remove from processing
    }

    // Find and build navigation from ul
    ...
    // Add remaining elements as tools/utility
